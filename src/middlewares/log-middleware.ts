...
export const logRequest = (request: TCustomRequest) => {
  ...
  Logger.log(`[${request?.traceId}][${dayjs().format()}][RECEIVED] ${request?.method} ${request?.url}`);
  return;
}

export const logResponse = (response: Response, request?: TCustomRequest): Response => {
  ...
  Logger.log(`[${request?.traceId}][${dayjs().format()}][COMPLETED] ${request?.method} ${request?.url} ${response.status} ${execTime.toFixed(2)}ms`);
...
export const logError = (error: Error & { status?: number }, request: Request & { traceId?: string }) => {
  ...
  Logger.log(`[${request?.traceId}][${dayjs().format()}][ERROR]`, JSON.stringify({
    error: error.message,
    stack: error.stack,
  }))
};
